...
class RiderAvailabilitySerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ("is_online", "schedule", "updated_at")
...
class RiderEarningsSerializer(serializers.ModelSerializer):
    class Meta:
        model = RiderEarnings
        fields = ("id", "period_start", "period_end", "total_deliveries", "total_earnings")
...
class RiderAcceptOrderSerializer(serializers.Serializer):
    ...
    def validate_order_id(self, value):
        if not Order.objects.filter(id=value, status=Order.Status.CONFIRMED, rider__isnull=True).exists():
            raise serializers.ValidationError("Order is not available.")
        return value
...
class InventoryItemSerializer(serializers.ModelSerializer):
    class Meta:
        model = InventoryItem
        fields = ("id", "branch", "name", "description", "price", "stock", "is_active")


class MerchantOrderStatusSerializer(serializers.Serializer):
    status = serializers.ChoiceField(
        choices=(
...
class AdminRiderKycSerializer(serializers.Serializer):
    kyc_status = serializers.ChoiceField(choices=("PENDING", "VERIFIED", "REJECTED"))


class AdminOrderReassignSerializer(serializers.Serializer):
    rider_id = serializers.IntegerField()

    def validate_rider_id(self, value):
        if not RiderProfile.objects.filter(id=value).exists():
            raise serializers.ValidationError("Rider not found.")
        return value


class AdminDeliveryFeeSerializer(serializers.Serializer):
